...
| F-1-4 | Checkout and verification now use only `https://api.sociobot.in/api/v1`. Returned tokens are stored, stripped from the URL, and verified. | `@claim:live-checkout`, `@claim:paid-unlimited`, `@claim:license-restore`, `@claim:license-revocation`; [checkout screenshot](evidence/checkout-live.png); live endpoint returned 303 to `checkout.dodopayments.com` |
